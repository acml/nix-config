...
(after! projectile
  ...
  (projectile-register-project-type 'cm12435 #'dinm5CG52813LW--cm12435-project-p
                                    ...
                                    :compile "set -o pipefail && mkdir -p log && unbuffer make -j$(nproc --ignore=1) -s |& tee log/build-$(date -Iseconds).log")
  ...
  (projectile-register-project-type 'git_dt (list dinm5CG52813LW-project-config-file)
                                    ...
                                    :compile "set -o pipefail && mkdir -p log && unbuffer ./docker_make.sh -j$(nproc --ignore=1) -s |& tee log/build-$(date -Iseconds).log"
